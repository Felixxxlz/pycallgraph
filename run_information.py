...
for caller in callgraph:
    ...
    for callee in callees:
        s.add(callee)
        if (caller.startswith(repo) or caller.startswith("tests.") or caller.startswith("test_")) and callee.startswith(repo):
            edges += 1
...
print(len([si for si in s if si.startswith(repo) or si.startswith("tests.") or si.startswith("test_")]))
