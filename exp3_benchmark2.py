...
def main():
    upstreams = ("pandas", "scipy", "astropy", "sklearn", "numpy")
    with open("exp3_config_related_downstreams2.json", mode="r") as rf:
        config = json.load(rf)
    for upstream in upstreams:
        ...
        conn = sqlite3.connect("benchmark_test_logs2/databases/test_info.db")
        for version in versions:
            ...
            try:
                os.makedirs(os.path.join("benchmark_test_logs2", upstream, version))
            except:
                pass
            ...
            for downstream in tqdm.tqdm(related_downstreams):
                ...
                log_path = os.path.join("benchmark_test_logs2", upstream, version, \
                    downstream + ".log")
