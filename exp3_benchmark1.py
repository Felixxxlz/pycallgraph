...
def main():
    upstreams = ("numpy", "pandas", "scipy", "astropy", "sklearn")
    with open("exp3_config_related_downstreams1.json", mode="r") as rf:
        config = json.load(rf)
    for upstream in upstreams:
        ...
        conn = sqlite3.connect("benchmark_test_logs1/databases/test_info.db")
        for version in versions:
            ...
            try:
                os.makedirs(os.path.join("benchmark_test_logs1", upstream, version))
            except:
                pass
            ...
            for downstream in tqdm.tqdm(related_downstreams):
                ...
                log_path = os.path.join("benchmark_test_logs1", upstream, version, \
                    downstream + ".log")
